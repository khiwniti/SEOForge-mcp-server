...
class UMCP_MCP_Client {
     ...
     */
    public function generate_content($topic = '', $keywords = [], $language = 'en', $tone = 'professional', $length = 'medium', $keywords_only = false) {
        // Support both old parameter format and new individual parameters
        if (is_array($topic)) {
            $params = $topic; // Old format: single array parameter
        } else {
            ...
            $params = [
                ...
                'include_images' => false // For backward compatibility
            ];
        ...
        // Use keyword-only endpoint if specified
        if (isset($params['keywords_only']) && $params['keywords_only'] && empty($params['topic'])) {
            $endpoint = '/universal-mcp/generate-from-keywords';
            // Simplify parameters for the new endpoint
            $simple_params = [
            ...
            return $this->make_request('POST', $endpoint, $simple_params);
        } else {
