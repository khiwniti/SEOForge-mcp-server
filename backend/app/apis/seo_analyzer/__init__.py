...
async def suggest_meta_tags_async(text: str, keywords: Optional[List[str]], current_title: Optional[str], current_description: Optional[str], model) -> MetaTagSuggestion:
    ...
    try:
        ...
        if not response_text:
            print("Gemini returned empty content for meta tags.")
        ...
        if not suggested_title and not suggested_description:
            print(f"Could not parse title/description from Gemini response: {response_text}")
        ...
        print(f"Error during Gemini call for meta tag suggestions: {e}")
...
async def generate_actionable_recommendations_async(
    ...
    if analysis_results.get("meta_tag_suggestions"):
        ...
        if meta.get("suggested_description"):
    ...
    try:
        ...
        response_text = response.text
        if not response_text:
            print("Gemini returned empty content for recommendations.")
        ...
        print(f"Error during Gemini call for actionable recommendations: {e}")
...
async def analyze_seo_content(request: SeoAnalysisRequest):
    ...
    # If no Gemini client, return mock analysis
    if gemini_model is None:
        print("No Gemini API key found, returning mock SEO analysis")
        
        # Mock keyword density
        density_results_list = []
        if request.keywords:
            for keyword in request.keywords:
                count = request.content.lower().count(keyword.lower())
                total_words = len(request.content.split())
                density = (count / total_words) * 100 if total_words > 0 else 0
                density_results_list.append(KeywordDensityResult(
                    keyword=keyword,
                    count=count,
        ...
        readability_scores_model = ReadabilityScores(
            flesch_reading_ease=65.0,
            flesch_kincaid_grade=8.5,
            gunning_fog=10.2
        )
        
        # Mock meta suggestions
        meta_suggestions_model = MetaTagSuggestion(
            suggested_title=f"Optimized: {request.content[:50]}..." if len(request.content) > 50 else request.content,
            suggested_description=f"Learn about {request.content[:100]}... Discover key insights and best practices."
        )
        
        # Mock recommendations
        recommendations = [
            "Optimize keyword density to 1-3% for better SEO performance",
            "Improve readability by using shorter sentences and paragraphs",
            "Add more relevant keywords naturally throughout the content",
            "Include internal and external links to authoritative sources",
            "Optimize meta title and description for better click-through rates"
        ]
    ...
    # Orchestrate analysis calls with real Gemini
    ...
    analysis_data_for_recommendations = {
        "keyword_density_results": [dr.dict() for dr in density_results_list] if density_results_list else [],
        "readability_scores": readability_scores_model.dict() if readability_scores_model else {},
        "meta_tag_suggestions": meta_suggestions_model.dict() if meta_suggestions_model else {},
        "original_content_length": len(request.content),
        "current_seo_score": overall_score # Pass the score for context to recommendations
    }

    recommendations = await generate_actionable_recommendations_async(analysis_data_for_recommendations, gemini_model)
