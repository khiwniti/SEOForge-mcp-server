...
class SEO_Forge_Chatbot {
	...
	public function __construct() {
		...
		// Admin actions for chatbot management
		add_action( 'wp_ajax_seo_forge_reset_chatbot', [ $this, 'reset_chatbot_settings' ] );
		add_action( 'wp_ajax_seo_forge_enable_chatbot', [ $this, 'enable_chatbot' ] );
	}

	/**
	 * Maybe force initialize chatbot settings.
	 */
	private function maybe_force_initialize() {
		if ( get_option( 'seo_forge_chatbot_force_init' ) ) {
			$this->ensure_chatbot_settings();
			delete_option( 'seo_forge_chatbot_force_init' );
		}
	}

	/**
	 * Ensure chatbot settings exist with proper defaults.
	 */
	private function ensure_chatbot_settings() {
		...
		// Log initialization
		error_log( 'SEO Forge: Chatbot settings initialized/updated' );
	}
	...
	private function should_show_chatbot() {
		...
		// Check page restrictions (empty array means all pages allowed)
		...
		// Check user role restrictions (empty array means all roles allowed)
		...
		if ( wp_is_mobile() && isset( $settings['show_on_mobile'] ) && ! $settings['show_on_mobile'] ) {
			return false;
		}
	 ...
	 */
	public function enable_chatbot() {
		check_ajax_referer( 'seo_forge_nonce', 'nonce' );
		
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( [ 'message' => __( 'Insufficient permissions.', 'seo-forge' ) ] );
		}
		...
		wp_send_json_success( [ 
			'message' => __( 'Chatbot has been enabled successfully!', 'seo-forge' ),
			'settings' => $settings
		] );
	}

	/**
	 * Reset chatbot settings via AJAX.
	 */
	public function reset_chatbot_settings() {
		check_ajax_referer( 'seo_forge_nonce', 'nonce' );
		...
		wp_send_json_success( [ 
			'message' => __( 'Chatbot settings have been reset to defaults!', 'seo-forge' ) 
		] );
	}
}